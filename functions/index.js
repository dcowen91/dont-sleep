...
async function getRosterInfo(leagueId, userId) {
  ...
  const userOwnedPlayers = teams.find(team => team.owner_id === Number(userId))
    .players;
...
exports.getRosterInfo = functions.https.onCall(async (data, _context) => {
  const response = await getRosterInfo(data.leagueId, data.userId);
  return response;
});
